...
async function deleteOldLogFiles(req, res, next) {
  try {
    ...
    const files = fileNames.map((fileName) => fs.stat(path.join(logDir, fileName))
    ...
    // которые должны быть удалены, если их время создания в файловой системе меньше, чем weekAgo
    await Promise.all(
      filesToRemove
    ...
    console.log('Clearing old logs'); // выводим сообщение в консоли

    next();
  } catch (err) {
    console.log(err.message); // ловим и выводим ошибки
